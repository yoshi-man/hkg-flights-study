...
def get_actual_timestamp(x) -> str:
    """
    A function to be applied to the dataframe to render the actual_timestamp as %Y-%m-%d %H:%M
    """
    try:
        if len(x['status'].split(' ')) > 1:
            timestamp = datetime.strptime(x['status'].split(
                ' ')[-1][1:-1], '%d/%m/%Y').strftime('%Y-%m-%d') + ' ' + x['status'].split(' ')[0]
        else:
            timestamp = x['date'] + ' ' + x['status']
...
def extract_transform_flights(normalized_df: pd.DataFrame, arrival: str) -> pd.DataFrame:
    ...
    df['airline'] = normalized_df['flight'].apply(
        lambda x: x[-1].get("airline")) if arrival == 'true' else normalized_df['flight'].apply(lambda x: x[0].get("airline"))
    ...
    df[actual_field_name] = pd.to_datetime(
        df.apply(get_actual_timestamp, axis=1), infer_datetime_format=True)
